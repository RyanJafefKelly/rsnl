...
def run_rsnl(
    ...
    init_params = {
        "theta": init_thetas,
        "adj_params": jnp.repeat(jnp.zeros(summary_dims), num_chains).reshape(
            num_chains, -1
        ),
    }
    ...
    for i in range(num_rounds):
        ...
        #             pool.starmap(
        #                 sim_fn,
        #                 [(sim_key, *theta) for sim_key, theta in zip(sim_keys, thetas)],
        #             )
        #         )
...
def run_snp(
    ...
    for i in range(num_rounds):
        ...
        if i == 0:
            ...
            thetas_standard = flow.sample(
                sub_key1, (num_sims_per_round,), x_obs_standard
            )
            thetas = (
                thetas_standard * standardisation_params["theta_std"]
                + standardisation_params["theta_mean"]
            )
    ...
    final_posterior_samples_standardised = flow.sample(
        sub_key, (num_final_posterior_samples,), x_obs_standard
    )

    final_posterior_samples = (
        final_posterior_samples_standardised * standardisation_params["theta_std"]
        + standardisation_params["theta_mean"]
    )
